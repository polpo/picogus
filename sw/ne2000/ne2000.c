...
uint8_t PG_EnableWifi() {
        ...
        printf("cyw43_wifi_set_up(&cyw43_state,CYW43_ITF_STA,true,CYW43_COUNTRY_CANADA);\n");        
        cyw43_wifi_set_up(&cyw43_state,CYW43_ITF_STA,true,CYW43_COUNTRY_CANADA);   
        cyw43_wifi_get_mac (&cyw43_state, CYW43_ITF_STA, (uint8_t *)&cyw43_mac);          
        printf("WIFI Address: %02x%02x.%02x%02x.%02x%02x\n\r",cyw43_mac[0],cyw43_mac[1],cyw43_mac[2],cyw43_mac[3],cyw43_mac[4],cyw43_mac[5]);    
        
        sprintf(WIFI_SSID,"OH");
        sprintf(WIFI_KEY,"YEAH");
        printf("Joining SSID: %s\n",WIFI_SSID);                                                
        int ConnectErr=cyw43_arch_wifi_connect_async(WIFI_SSID, WIFI_KEY, CYW43_AUTH_WPA2_MIXED_PSK);
        if (ConnectErr==0) printf("Connection Ok ! \n");
...
void *ne2000_common_init() {        
        ...
        memcpy(ne2000->physaddr, cyw43_mac, 6);
